...
Each organism stores its age and lifecycle state, concrete internal resource accounts, current tile, within-tile position, velocity, current behavior, and any small capability state that must persist across ticks. Current environmental stress and health are derived from this concrete state, species DNA, and the surroundings rather than existing as unrelated consumable pools. The simulation materializes each gameplay-relevant result once at its named update boundary and stores that result for every downstream process, explanation, save, and client view; consumers do not independently recalculate it.

Behavior captures what the organism is currently trying to do. Primitive organisms may move or feed with little direction, while more advanced organisms may switch behaviors based on age, health, resource reserves, recent acquisition success, sensed gradients, nearby organisms, and environmental conditions. Behavior is local and reactive: an organism may use only its own state, its recent experience, and information exposed by its DNA-encoded senses. It does not receive free knowledge of global population, hidden tiles, or future conditions.

Resource pressure emerges from finite resources and contention rather than an artificial carrying-capacity rule. Basic physiology prevents invalid or unaffordable transactions, while evolved behavioral regulation can conserve reserves, defer optional growth or reproduction, pursue an observed food source, flee a threat, or disperse from an unfavorable tile. Conservation should suppress avoidable costs, not mandatory maintenance or free useful passive acquisition. Available behaviors, persistence, sensing requirements, and selection rules are encoded by DNA; future traits may add costly density-sensitive signaling or stochastic quiescence.

Although species-wide aggregates do not inform individual behavior, the simulation and interface should summarize the current behavior states of a species as counts or proportions, including per-tile distributions where visible. These summaries are observational outputs derived from individual states, not shared knowledge available to the organisms.
