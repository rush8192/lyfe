...
Organisms without active movement capabilities still experience small random Brownian displacement by default. Its expected direction is zero, so it creates diffusion rather than a consistent current or preferred direction. Brownian displacement may carry an organism to a tile edge, but any crossing remains subject to the same boundary, habitat, and migration rules as other movement. Evolved locomotion adds directional control and greater displacement rather than being the sole reason coordinates can change.
