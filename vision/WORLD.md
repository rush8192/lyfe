...
- A spatially coherent lithology or exposed-rock profile that influences which finite inorganic and micronutrient stocks are locally abundant or scarce.
...
Within-tile positions determine which organisms and remnants are close enough for direct interactions such as predation and scavenging. These direct interactions never cross a tile boundary: an organism must first migrate and become part of the neighboring tile before interacting with entities there. An organism must be sufficiently near an edge shared with a neighboring tile before it can migrate into that tile. Its capabilities and the compatibility of the two environments determine whether and how readily it crosses.

Organisms without active movement capabilities still experience small random Brownian-like displacement by default. Its expected direction is zero, so it creates diffusion rather than a consistent current or preferred direction. Body scale, medium, lifecycle state, and DNA-defined environmental coupling may alter its magnitude: anchoring reduces passive displacement, while a drifting strategy increases it. Passive displacement may carry an organism to a tile edge, but any crossing remains subject to the same boundary, habitat, and migration rules as other movement. Evolved locomotion adds directional control and greater displacement rather than being the sole reason coordinates can change.

The world may later provide directional transport through water currents, wind, buoyancy, sinking, or attachment to moving environmental carriers. Organisms can evolve lifecycle or structural adaptations that exploit those processes as a passive dispersal strategy. Environmental transport moves organism entities rather than copying them, remains local and edge-crossing, and cannot bypass tile compatibility or survival limits. V1 does not simulate directional currents or wind; its DNA-modified zero-mean passive movement, reproduction, boundary trace, and migration rules provide the initial foundation for population-driven passive spread.

For v1, direct organism/remnant interaction is the primary simulation purpose of within-tile coordinates. Abiotic environmental resources, gases, temperature, light, and other tile conditions do not become more or less accessible because an organism is at a particular coordinate inside the tile. Coordinates may later gain additional meaning if localized resource fields, currents, terrain features, or other within-tile structures are introduced.
...
World generation and tile initialization should be driven by coherent, named data profiles. A profile groups geography, climate, seasonality, volcanism, atmospheric backgrounds, lithology, initial resource distributions, seeded variation, and starting-region guarantees so changing the kind of world does not require editing simulation code or assembling unrelated sliders. The official primordial-Earth-like world is one such profile rather than a privileged generator path.

The architecture should permit additional official profiles and externally supplied data-only world packs. These packs may configure registered generation algorithms and existing environmental/resource concepts, but do not implicitly add executable generator code or new biological vocabulary. A generated world stores the exact pack, profile, selected options, and seed that produced it. Different profiles should create materially different distributions of opportunity and pressure, allowing life to follow different emergent evolutionary histories while using the same underlying biological rules.
...
Terrestrial environments are not intrinsically more productive than aquatic ones. They can offer unattenuated surface light, direct access to atmospheric gases, sparsely occupied space, and locally valuable mineral endowments, while imposing stronger moisture, temperature, movement, and later weather pressures. Rain and surface moisture may change organisms' access to matter already present in a tile, but never create that matter. Different rock regions should expose different nutrient strengths and shortages so land exploration reveals alternative niches rather than a universal upgrade. The first terrestrial primary producers can create biomass and recycled organic opportunities that later support consumers and predators through the same mass-balanced lifecycle used in water.
