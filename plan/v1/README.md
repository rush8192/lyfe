...
| [ENTITY_IDENTITY_AND_STORAGE.md](ENTITY_IDENTITY_AND_STORAGE.md) | How do stable IDs, transactional pages, tile-partitioned dense stores, locators, and derived indexes represent the authoritative world? | World ownership, data model, synchronization |
...
- Encode the first representative rule bundle and assign its permanent definition/RNG/outcome/event/mod/world-parameter IDs. Strict JSON authoring, generated schemas, typed registries and tombstones, normalization, hash identities, fail-closed validation, global compilation, and full deterministic phenotype compilation are fixed in [RULE_PACK_AUTHORING_AND_COMPILATION.md](RULE_PACK_AUTHORING_AND_COMPILATION.md). The official biological pack is the ordinary zero-mod base and the primordial-Earth-like setup is an ordinary complete world pack; [MODDABILITY.md](MODDABILITY.md) defines local balance overlays, selectable external world profiles, conflict rejection, final identities, certification, and the boundary for future general content mods. The `Philox4x64-10` semantic-address contract, scalar oracle, worker isolation, exact reduction, canonical creation/ID assignment, owner-only commit, transactional pages, tile-partitioned dense storage, hot resource matrices, phase-specific tile views, locators, derived indexes, and mutation/change capture are likewise fixed at a first-pass level; concrete types and physical tuning remain prototype work.
- Final date, speed presets, complete command/state-machine catalogues, save/checkpoint format, protocol code generation, and exact client projection schemas. Revision, coalescing, atomic apply, visibility replacement, and resynchronization semantics are now fixed in [STATE_CHANGE_AND_CLIENT_SYNC.md](STATE_CHANGE_AND_CLIENT_SYNC.md).
