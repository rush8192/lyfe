...
| [FOUNDER_SETUP_PACKAGES.md](FOUNDER_SETUP_PACKAGES.md) | Which bounded efficiency-versus-chemical-tolerance allocations make the initial DNA choice meaningful without creating a superior founder? | Founding metabolisms, health, generated-world validation, setup |
...
- The first player-loop contract is defined in [PLAYER_LOOP_AND_NARRATIVE.md](PLAYER_LOOP_AND_NARRATIVE.md). The complete generated-world opening passes all six metabolism/allocation choices across the recorded 64-seed matrix with persistent micronutrient quotas, and UI-200/UI-205 implement its observation, decision, recovery, camera, and local-preference surfaces. The immediate product gate is representative end-to-end `PERF-200` profiling; structured internal alpha follows, with copy and interface polish deliberately driven by attributable playtest findings.
- The bounded efficiency-versus-tolerance packages are fixed and accepted for the executable opening. The remaining player-facing pacing decision before the first representative rule pack freezes is the default simulation deadline plus final-tick loss/completion precedence; the scheduler, range proof, and final gameplay benchmark must consume explicit values rather than infer them.
- The deterministic `32 × 17` generated-world slice now has representative opening-biological validation in addition to terrain, climate normals, x-only topology, volcanic bands, start repair, gas flow, and replay. Recurring weather/moisture, lithology, and broader non-gas resource placement remain in their owning later slices.
- Core body scale, Brownian/drift motion, sensing, local interaction, migration, pressure channels, and conservation behavior are executable. The next spatial/behavior risks are target-driven foraging, fleeing, advanced dispersal, terrestrial movement, and clustered representative-scale profiling.
- Founder lifecycle mechanics—including fission, health, senescence risk, death remnants, scavenging, digestion, decay, and exact micronutrient recycling—are executable. Coupled organic mortality/decay ecology, terrestrial dormancy, predation, and later strategies still require population fixtures.
...
- Expand the implemented strict rule/world bundles without weakening their permanent-ID, tombstone, normalization, hash, validation, and compilation contracts. The official biological pack remains the ordinary zero-mod base and world generation remains a selectable complete pack; local balance overlays and external-pack certification are still planned. The scalar oracle and ownership/storage foundations are executable, while measured parallel phase execution and physical tuning remain later performance work.
