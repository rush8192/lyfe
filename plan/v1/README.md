...
- Final migration costs/probabilities, baseline behavior selection, and clustered spatial-performance validation. First body, Brownian, sensing, interaction, placement, active-distance, acceleration, turning, and movement-energy values are specified in [SPATIAL_CALIBRATION.md](SPATIAL_CALIBRATION.md).
- Executable multi-seed validation of the complete first lifecycle rule pack: the organic mortality/decay niche and autonomous material-opportunity score now have authoring landmarks in [POPULATION_ECOSYSTEM_VALIDATION.md](POPULATION_ECOSYSTEM_VALIDATION.md), while dormancy, scavenging, digestion, and later ecological strategies still need coupled population fixtures.
...
- Execute the first complex-organization, dissolved-organic-specialist, and predator population fixtures; terrestrial adaptation remains the final unexpanded mid/late-game rule pack. Complex-cell processing loads, organization budgets, structural assignments, assembly ceilings, scale liabilities, particulate quotas, movement profiles, specialized LDO access, and analytical prey-density thresholds are now selected. Coupled executable ecosystem validation remains.
