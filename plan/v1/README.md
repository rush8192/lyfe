...
- A representative benchmark rule pack, reference hardware, fastest-speed threshold, concrete state-hash field tags/golden vectors, and performance-regression policy. The logical `WorldStateHashV1` scope, ordering, algorithm, load verification, and cadence are now fixed in [PERSISTENCE_AND_REPLAY.md](PERSISTENCE_AND_REPLAY.md).
