...
Unused MP and the fractional income remainder are intentionally copied after price payment, not divided. Pressure history is also copied because both branches experienced it before the fork; subsequent updates diverge. Organism positions, age, reserves, matter, bindings, lifecycle state, reproduction/scavenging cooldowns, and behavior persist. Invalid behavior or target capabilities are cleared through the normal species-transition rule, and new capacities begin empty. Speciation never creates resources, repairs health, advances a lifecycle phase, or grants construction matter.
