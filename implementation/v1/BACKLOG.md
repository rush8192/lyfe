...
Stage A is complete, and `WORLD-200` now supplies the first deterministic generated-world artifact. The next implementation item is `ORG-200`: organism state and health can now use representative generated tile/climate fixtures. `GAS-200` is also dependency-ready and follows once those internal organism accounts exist to consume its resource fields.
...
| RNG-100 | Done | Implement versioned `Philox4x64-10` semantic keyed randomness | FND-010, STATE-100 | Official and independent-reference golden vectors, a frozen 23-domain manifest, exact integer conversions, misuse failures, and call/enumeration-order invariance pass in Debug and Release |
...
| GAS-200 | Ready | Implement tile atmospheric stocks, mixing, volcanic sources, and sinks | WORLD-200 | Volcanic plateaus and neighbor concentrations meet calibrated scenario bands |
| ORG-200 | Ready | Implement calibrated internal state, computed health, intrinsic death evidence, and storage/allocation | STAGE-A | Health explanations have one materialized source and reserve/resource invariants pass |
| SPACE-200 | Planned | Implement coordinates, body radius, Brownian/drift movement, index, local interactions, and edge migration | WORLD-200, ORG-200 | No cross-tile interaction; scalar fixtures satisfy range and migration invariants |
| LIFE-200 | Planned | Implement deterministic reproduction with cooldown jitter, senescence, remains, scavenging/digestion, decay, and recycling | SPACE-200, ORG-200 | Reproduction is mass-balanced; all nonzero death probabilities are recorded at death; lifecycle fixture closes |
| BEHAVIOR-200 | Planned | Implement persistent local behavior and resource-pressure regulation | LIFE-200 | Decisions use organism-local observations only; aggregate/per-tile species behavior is projectable |
| EVO-200 | Planned | Implement mutation income, trait trees, compiled phenotype updates, speciation, lineage, and autonomous opportunity scoring | BEHAVIOR-200 | Controlled/uncontrolled fixtures, founder fractions, retained points, and lineage invariants pass |
| GAME-200 | Planned | Implement survival and sandbox setup/control/loss state machines | EVO-200, DELTA-100 | Both v1 modes complete setup-to-loss flows; multiplayer assumptions do not leak into ownership boundaries |
| OPENING-200 | Planned | Implement hydrogen and sulfide founder paths plus bounded starting trade-off packages | GAME-200, GAS-200 | Both choices expose multiple legible non-dominated first decisions across the expanded seed matrix |
| UI-200 | Planned | Add knowledge-limited map, organism/species inspection, mutation decisions, behavior summaries, and resource-flow views | OPENING-200 | Player can observe, diagnose, choose, and review consequences without hidden-state leakage |
