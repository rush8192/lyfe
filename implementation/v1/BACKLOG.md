...
The first long-run resource-cycle playtest found a separate Stage-B defect: ordinary remnant
structure conserved phosphorus inside a generic organic residue that had no executable return path,
while the authored geological phosphorus input was absent from generated worlds. `RECYCLE-210`
closes that playable-opening phosphorus loop without claiming the broader organic-ecology
mineralization network owned by `BIO-300`.
...
current climate and compares overall environmental fit, temperature, habitat medium, and light
requirements with the controlled species' compiled DNA. It filters compounds to metabolic, growth,
micronutrient, and chemical-hazard relevance; missing needs and tolerance violations are explicit,
while abundance is claimed only against the last observed controlled-species uptake rate. Selectable
`72`/`168`/`336`/`720`-tick mortality summaries group actual controlled-species death mechanisms by tile.
Reduced observation age still comes from authoritative tick boundaries, reduced tiles list only
retained resource identities, and unknown tiles withhold all unavailable detail. Explicit selection
of a reduced or unknown tile never substitutes unrelated live evidence.
...
| RECYCLE-210 | Done | Close the playable-opening remnant and phosphorus recycling loop | LIFE-200, OPENING-200, GAS-200 | Exact remnant-decay ledger transactions expose structural, reserve, and terminal micronutrient return; fully exhausted remnants are removed under a bounded regression; structural decay releases its phosphorus as usable inorganic phosphorus while conserving all remaining atoms in a distinct phosphorus-depleted residue; the generated-world profile supplies the authored `250` inorganic-phosphorus units per tile-hour as an exact boundary source; save/reload, rule/world identity, flow projection, and a long-run phosphorus fixture remain deterministic and mass balanced; slow full CHNOS mineralization of depleted/spent organic pools remains explicitly assigned to `BIO-300` |
