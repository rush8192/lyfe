...
- The persistence boundary encodes bounded envelope/payload schema 10, including founder allocation, abiogenesis roots, persistent micronutrient inventories, and the append-only organism journey stream. Load checks compatibility before the payload, reconstructs runtime stores and indexes, recompiles and verifies allocation-aware genomes, reconciles the ledger, verifies the recorded `WorldStateHashV9`, and resumes with an identical next tick and event identity continuation. The atomic writer preserves the prior save across an injected pre-replace failure.
- Successful tick commits derive canonical birth, reproduction, resource-absorption, feeding, migration, and death facts from lifecycle receipts, relocations, and the balanced resource ledger. Each event receives a stable world-local ID only after the whole tick succeeds; death landmarks retain every nonzero cause probability plus the triggered cause. The actor projector currently authorizes the controlled species' events. Stress/behavior-transition facts and hourly/daily routine-event compaction remain in `UI-200`.
- Versioned `lyfe.v1` Protocol Buffer sources generate C# and TypeScript bindings with exact 64-bit integer semantics. The server maps the actor-authorized unknown/reduced/live projection, controlled-species evolution, gameplay state, and journey events to a binary `ProjectionSnapshot` at `/api/v1/worlds/active/projection`; deltas append journey identities without coalescing, and authoritative state hashes are not exposed.
- The React/PixiJS client decodes the generated snapshot, stores `bigint` identities and quantities, and renders every live projected organism one-for-one. Its normalized cache atomically applies absolute gameplay/whole-tile/species batches plus ordered journey-event appends, ignores duplicates, and requests a full resync on gaps, ordering, identity, rules, or structural failures without changing the committed view. Configurable shape-and-color activity pulses fade on wall-clock time, and the first Journey inspector switches among visible controlled organisms.
